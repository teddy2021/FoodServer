...
void DBConnector::connect() {
	Logger::GetInstance().log("[DBConnector::connect] Establishing connection", debug_level::INFO);
	connection = std::unique_ptr<sql::Connection>(
		driver->connect(connectionUrl, connectionProperties));
	if (!connection) {
		throw std::runtime_error("[DBConnector::connect] Failed to create connection");
	}
}
...
bool DBConnector::reconnect(int maxRetries) {
	for (int attempt = 0; attempt < maxRetries; ++attempt) {
		try {
			connect();
			if (isConnected()) {
				Logger::GetInstance().log("[DBConnector::reconnect] Reconnected successfully", debug_level::INFO);
				return true;
			}
		} catch (std::exception& e) {
			Logger::GetInstance().log("[DBConnector::reconnect] Attempt " + std::to_string(attempt + 1) + " failed: " + std::string(e.what()), debug_level::ERROR);
		}
		int delay = 1000 * (1 << attempt);
		Logger::GetInstance().log("[DBConnector::reconnect] Waiting " + std::to_string(delay) + "ms before retry", debug_level::DEBUG);
...
void DBConnector::ensureConnected() {
	if (!isConnected()) {
		Logger::GetInstance().log("[DBConnector::ensureConnected] Connection lost, attempting reconnect", debug_level::WARN);
		if (!reconnect()) {
			throw std::runtime_error("[DBConnector::ensureConnected] Failed to reconnect to database after retries");
		}
	}
}
...
string DBConnector::GetIngredientByIndex(int index){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::GetIngredientByIndex] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::GetIngredientByIndex] Failed after retries");
}
...
void DBConnector::CreateIngredient(std::string name, float amount, bool mass){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::CreateIngredient] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::CreateIngredient] Failed after retries");
}
...
void DBConnector::CreateRecipe(string name, string instruction, string url){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::CreateRecipe] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::CreateRecipe] Failed after retries");
}
...
void DBConnector::MapRecipeToIngredient(string recipe, float amount, string ingredient){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::MapRecipeToIngredient] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::MapRecipeToIngredient] Failed after retries");
}
...
void DBConnector::UpdateIngredient(std::string name, float amount){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::UpdateIngredient] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::UpdateIngredient] Failed after retries");
}
...
void DBConnector::DeleteIngredient(std::string name){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::DeleteIngredient] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::DeleteIngredient] Failed after retries");
}
...
void DBConnector::DeleteRecipe(std::string name){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			if(name.empty() || name.find_first_not_of(" ") == std::string::npos){
				throw std::runtime_error("[DBConnector::DeleteRecipe]: name cannot be empty.");
			}
			...
			Logger::GetInstance().log("[DBConnector::DeleteRecipe] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::DeleteRecipe] Failed after retries");
}
...
vector<pair<int,float>> DBConnector::Reserve(vector<string> ingredients, vector<float> amounts){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::Reserve] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::Reserve] Failed after retries");
}
...
DBConnector::DBConnector(){
	...
	connectionUrl = "jdbc:mariadb://localhost:3306/FoodServer?charset=utf8mb4";
	connectionProperties = sql::Properties({
			{"user", "foodserver"},
			{"password", res}});
	connect();
}
...
vector<string> DBConnector::GetIngredients(std::string recipe){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::GetIngredients] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::GetIngredients] Failed after retries");
}
...
vector<string> DBConnector::GetRecipes(vector<string> ingredients){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::GetRecipes] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::GetRecipes] Failed after retries");
}
...
vector<string> DBConnector::GetInstructions(vector<string> recipes){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::GetInstructions] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::GetInstructions] Failed after retries");
}
...
void DBConnector::Release(vector<string> items, vector<float> amounts){
	...
	for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
		try {
			...
			Logger::GetInstance().log("[DBConnector::Release] DB error: " + std::string(e.what()), debug_level::ERROR);
			if (attempt == MAX_ATTEMPTS - 1) throw;
		}
	}
	throw std::runtime_error("[DBConnector::Release] Failed after retries");
}
